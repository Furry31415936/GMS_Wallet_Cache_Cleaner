...
case "$1" in
    ...
    *)
        ...
        echo "清理操作（按风险从低到高）:"
        echo "  $0 safe           🟢 安全模式"
        echo "  仅清 cache/code_cache，最安全"
        echo ""
        echo "  $0 refresh        🟡 刷新模式"
        echo "  清缓存 + 杀unstable进程，推荐"
        echo ""
        echo "  $0 experimental   🔴 探索模式"
        echo "  额外清理 files/ 和 no_backup/"
        echo ""
        echo "辅助功能:"
        echo "  $0 backup         📦 全量备份"
        echo "  $0 snapshot       📸 文件快照"
        echo "  $0 diff           📊 对比快照"
        echo "  $0 list           📋 查看备份/快照"
        echo ""
        echo "示例:"
        echo "  $0 safe          日常清理"
        echo "  $0 refresh       刷新状态后重启"
        echo "  $0 snapshot      记录当前状态"
        echo ""
        ;;
esac
